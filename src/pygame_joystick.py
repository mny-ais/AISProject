...
while key != ord('q'):
    ...
    if num_joysticks > 0:
        ...
        for joy_num in range(num_joysticks):
            ...
            # State its name
            stdscr.addstr(num_lines, 0, "Name: {0}".format(joysticks[joy_num].get_name()))
            num_lines += 1
            ...
            stdscr.addstr(num_lines, 0, "Joystick has {0} axes".format(num_axis))
            num_lines += 1

            if num_axis > 0:
                for num in range(num_axis):
                    stdscr.addstr(num_lines, 0, "Axis {0}: {1}".format(num, joysticks[joy_num].get_axis(num)))
                    num_lines += 1

            # Get the number of buttons and state their state
            num_buttons = joysticks[joy_num].get_numbuttons()
            stdscr.addstr(num_lines, 0, "Joystick has {0} buttons".format(num_buttons))
            num_lines += 1

            if num_buttons > 0:
                for num in range(num_buttons):
                    stdscr.addstr(num_lines, 0, "Button {0}: {1}".format(num, joysticks[joy_num].get_button(num)))
                    num_lines += 1

            # Get the number of hats and state their state
            num_hats = joysticks[joy_num].get_numhats()
            stdscr.addstr(num_lines, 0, "Joystick has {0} hats".format(num_hats))
            num_lines += 1

            if num_hats > 0:
                for num in range(num_hats):
                    stdscr.addstr(num_hats, 0, "Hat {0}: {1}".format(num, joysticks[joy_num].get_hat(num)))
